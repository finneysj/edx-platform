...
def setup_acceptance_db(system, fasttest=false)
    if File.exists?(ACCEPTANCE_DB)
        if fasttest
            sh(django_admin(system, 'acceptance', 'flush', '--noinput'))
        else
...
task :clean_test_files do
    ...
    sh("git clean -fqdx test_root/logs")
    sh("git clean -fqdx test_root/data")
    sh("git clean -fqdx test_root/staticfiles")
    sh("git clean -fqdx test_root/uploads")
end
...
[:lms, :cms].each do |system|
    ...
    task "test_acceptance_#{system}", [:harvest_args] => [:clean_test_files, "#{system}:gather_assets:acceptance_static"] do
        setup_acceptance_db(system)
        Rake::Task["fasttest_acceptance_#{system}"].invoke()
    end
